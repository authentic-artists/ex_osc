defmodule OSC.Message do
  ...
  defp parse_bundle_element(
         <<elt_size::size(32)-signed, data::size(elt_size)-unit(8)-binary, rest::binary>>,
         acc
       ) do
  ...
  defp sntp_time_to_now(sec, usec) do
    ...
    {div(t, 1_000_000), rem(t, 1_000_000), round(usec * 1_000_000 / bsl(1, 32))}
  end
  ...
  def construct(:osc_bundle, {:osc_timetag, datetime}, data) do
    ...
    {secs, usecs} =
      case datetime do
        {{_, _, _}, {_, _, _}} ->
          datetime |> datetime_to_now |> now_to_sntp_time

        microseconds_since_epoch when is_integer(microseconds_since_epoch) ->
          # i.e. from System.os_time(:microsecond)
          microseconds_since_epoch_to_sntp_time(microseconds_since_epoch)
      end
  ...
  defp construct_args({:osc_rgba, colors}, {tags, args}) do
    {tags <> <<?r>>,
     args <>
       <<value_or_zero(colors[:red]), value_or_zero(colors[:green]), value_or_zero(colors[:blue]),
         value_or_zero(colors[:alpha])>>}
  end

  defp construct_args({:osc_midi, values}, {tags, args}) do
    {tags <> <<?m>>,
     args <> <<values[:port_id], values[:status], values[:data1], values[:data2]>>}
  end
  ...
  defp datetime_to_now(datetime) do
    seconds = :calendar.datetime_to_gregorian_seconds(datetime) - 62_167_219_200
    ## 62167219200 == calendar:datetime_to_gregorian_seconds({{1970, 1, 1}, {0, 0, 0}})
    {div(seconds, 1_000_000), rem(seconds, 1_000_000), 0}
  end

  def microseconds_since_epoch_to_sntp_time(microseconds_since_epoch) do
    # 2_208_988_800_000_000 == 1_000_000 * NaiveDateTime.diff(~N[1970-01-01 00:00:00], ~N[1900-01-01 00:00:00])
    microseconds_since_Jan1900 = microseconds_since_epoch + 2_208_988_800_000_000
    secs = floor(microseconds_since_Jan1900 / 1_000_000)
    usecs = microseconds_since_Jan1900 - 1_000_000 * secs
    {secs, round(usecs * bsl(1, 32) / 1_000_000)}
  end
end
